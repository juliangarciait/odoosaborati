...
class WooSynchro(models.TransientModel):
    ...
    def synchro(self, data, query, server, table, accion, id_vex, api=None, queryx='',default_code=None):
        ...
        if query == "products":
            if default_code :
                ...
                domain = [('server_vex','=',server.id),('id_vex','=',id_vex)]
       ...
       # raise ValueError(exist)
        ...
        if not exist and   create_tmp:
            ...
            #raise ValueError([sql_fields['create'],data])
        ...
        if not exist and has_create:
            ...
            #exist = self.env[table].search(domain)
            raise ValueError([self.env.company.name,create_tmp,'no se encontro' + str(exist) + ':' + str(table) + ',' + str(domain) + '/' + str(sql_fields['create']),data])
        ...
        if customer_id != 0 or customer_id != None:
            ...
            if not partner:
                ...
                if not data['customer']['name'] or  not partner.name or  partner.name.find('Mercado Libre') != -1 :
                    ...
                    if 'billing' in data and 'name' in data['billing']:
            ...
            if 'billing' in data and data['billing']:
                ...
                #        if data['billing']['city'] and data['billing']['city'] != '':
    ...
    def check_product_order(self, p, server, accion,data):
        ...
        if atributo:
            ...
            if 'seller_sku' in p:
                ...
                if not pp:
                    if server.products_reemplace:
                        for re in server.products_reemplace:
                            if re.code == p['seller_sku']:
                                pp = re.product_id
            ...
            if server.search_sku and p['seller_sku']:
                ...
                if not pp:
                    ...
                    pp = self.env['product.product'].sudo().search([('default_code', '=', skux)])
                    ...
                    if not pp:
                        ...
                        if not pp:
                            ...
                            if server.create_not_exists:
                                ...
                                if not pp:
                                    raise ValueError(['No se encontro el Producto'+str(pp),p])
    ...
    def check_produc(self, id, server, accion,varition=None):
        ...
        exist_conector = self.env['vex.product.product.conector'].search([
            ('id_vex_varition', '=', id), ('instance', '=', server.id)
        ])
        # raise ValidationError(existe)
        if exist_conector:
            pro_id = exist_conector.product_id
        else:
            accionx = self.env['vex.restapi.list'].search([('argument','=','products'),('conector','=',server.conector)])
            ...
            pro_id = exist_conector.product_id if exist_conector else None
