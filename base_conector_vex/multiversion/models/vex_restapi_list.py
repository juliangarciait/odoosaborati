...
class VexRestapilist(models.Model):
    ...
    def go_action_list(self):
        ...
        if str(self.model) == 'sale.order':
            ...
            #orders = self.env['sale.order'].search([('conector', '=', self.conector)])
            #cliente_ids = []
            #for orden in orders:
            #    cliente_ids.append(orden.partner_id.id)
            #    cliente_ids.append(orden.partner_invoice_id.id)
            #sql = f'''  UPDATE  res_partner SET  valid_customer_vex = 't'  WHERE id in {tuple(cliente_ids)} '''
