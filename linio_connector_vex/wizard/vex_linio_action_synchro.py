...
class LinioActionSynchro(models.TransientModel):
    ...
    def check_product_order_linio(self,p  , server , accion , order):
        ...
        if server.search_sku:
            ...
            if not pp:
                if '_DELETED_' in p['Sku']:
                    sku = p['Sku'].split('_DELETED_')[0]
                    dmi = [(identi, '=', sku)]
                    pp = self.env['product.product'].search(dmi)
                    if not pp:
                        raise ValidationError(f'''NO SE ENCONTRO EL PRODUCTO con SKU X LINIO : {sku}  , {p['Sku']} , 
                                                            ORDER = {order.id_vex} , server : {server.display_name}  , {str(p)}''')
    ...
    def insert_lines(self,lines,server,creado , accion):
        if server.conector == 'linio':
            ...
            for p in lines:
                ...
                #atributo = p['Variation']]
                existe = self.check_product_order_linio(p, server, accion,creado)
    ...
    def start_sync_stock_linio(self):
        products = self.env['product.product'].search(['|',('id_vex','!=',False),('id_vex_varition','!=',False)],limit=150)
