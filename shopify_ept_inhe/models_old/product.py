...
class ProductTemplate(models.Model):
    ...
    #     """
         ...
         if vals["additional_cost_ids"] or vals["replacement_cost"]:
            self.update_prices_in_shopify()
    #     for product in self:
    ...
    def update_prices_in_shopify(self): 
        if self.shopify_product_template_ids:
            for product in self.shopify_product_template_ids: 
                export_data = self.env['shopify.process.import.export'].create({
    ...
    @api.onchange('replacement_cost')  
    def _replacement_cost_change(self):
        self.update_prices_in_shopify()
