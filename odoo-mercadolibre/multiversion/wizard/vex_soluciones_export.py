...
class MeliMultiExport(models.TransientModel):
    ...
    def export_product(self,conector,self2=None,validate=True):




        p = conector.product_id
        name_product = p.name_product_meli
        server = conector.instance
        if validate:
        ...
        if server.pricelist and server.update_price:
        ...
        if conector:
            ...
            id_vex = p.id_vex
            url = f'https://api.mercadolibre.com/items/{conector.id_vex}?access_token=' + str(server.access_token)
            data = {}
            ...
            if conector.id_vex_varition != conector.id_vex:
                datavaritians = self.env['vex.synchro'].get_data_id(conector.id_vex,server,"products")
                #raise ValueError(datavaritians)
                ...
                for vd in datavaritians:
                    ...
                    if logistic_or_server:
                        ...
                        if logistic_or_server.update_stock:
                            if conector.id_vex_varition == vd['id']:
                                if self2:
                                ...
                                exist_conector = self.env['vex.product.product.conector'].search([
                                    ('id_vex_varition', '=', conector.id_vex_varition),
                                    ('instance', '=', server.id)
                                ],limit=1)

                                if exist_conector:
                                    stockx = exist_conector.product_id.stock_vex_conector(server)
            ...
            if server.update_description:
                url_desc = f'https://api.mercadolibre.com/items/{p.id_vex}/description?access_token=' + str(
                    server.access_token)
                data_des = {
...
class MeliUnitExport(models.TransientModel):
    ...
    def start_export(self):
        ...
        exist_conector = self.env['vex.product.product.conector'].search([
            ('product_id', '=', self.product.id), ('instance', '=', self.server.id)
        ])
        if not exist_conector:
            raise ValidationError('NO EXISTE CONECTOR de la instancia')
        if len(exist_conector) > 1:
            for l in exist_conector:
                #if l.id_vex != l.id_vex_varition:
                self.env['meli.export'].export_product(l, self, False)
        else:
            self.env['meli.export'].export_product(exist_conector, self, False)
